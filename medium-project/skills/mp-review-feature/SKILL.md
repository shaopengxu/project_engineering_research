...
1. 搜索现有 Issue：
   `gh issue list --label "type:feature-review" --label "module:{module}" --search "Feature Review: {module}/{feature} in:title" --state open --json number --jq '.[0].number'`
2. 如果未找到，创建：
   `gh issue create --title "Feature Review: {module}/{feature}" --label "type:feature-review,module:{module}" --body "跟踪 {module}/{feature} Feature 的 Review 过程。"`
