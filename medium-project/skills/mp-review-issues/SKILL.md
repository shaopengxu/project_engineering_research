...
1. 搜索现有 Issue：`gh issue list --label "type:task-split" --search "任务拆分 in:title" --state open --json number --jq '.[0].number'`
2. 如果未找到，创建：`gh issue create --title "任务拆分" --label "type:task-split" --body "跟踪任务拆分的 Review 过程。"`
