...
8. **日志约定**：
   ...
   - 日志格式（JSON 结构化，含 requestId、module、timestamp）
   - 日志规则（requestId 贯穿调用链、敏感信息禁止入日志、级别通过环境变量控制）
9. **接口通用约定**：
   - 认证方式
   - 响应格式（成功/失败统一结构）
   - 分页约定
   - HTTP 状态码约定
10. **外部依赖清单**（如有第三方 API 依赖）：
   - 依赖名称和用途（如支付网关、短信服务、地图 API 等）
   - 是否有 sandbox/test 模式
   - 需要的环境变量（API key、secret 等）
   - 各测试层级的 Mock 策略：契约测试/L1 用 Mock，L2 可选 sandbox，E2E 用 sandbox
11. **接口依赖矩阵**（留空，所有模块设计完成后汇总填写）
11. **需求追溯表**（留空，所有模块设计完成后汇总填写）
12. **部署概要**（可选，如部署架构影响模块设计则必须说明）
