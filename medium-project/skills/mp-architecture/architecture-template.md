...
> 所有模块统一的日志策略，由 infra 提供基础能力。

### 日志级别

| 级别 | 用途 | 示例 |
|------|------|------|
| error | 需要关注的异常 | 未捕获异常、外部服务调用失败 |
| warn | 潜在问题但不影响主流程 | 参数校验失败、降级处理 |
| info | 关键业务事件 | 请求进出、状态变更、重要操作 |
| debug | 开发调试信息 | SQL 查询、变量值（生产环境关闭） |
...
- 每个请求通过中间件注入 `requestId`，贯穿整个调用链
- 日志输出到 stdout（JSON 格式），由运行环境负责收集
...
> 如无第三方 API 依赖可标注"无"并跳过此节。

| 依赖名称 | 用途 | 有 sandbox/test 模式 | 环境变量 | Mock 策略 |
|---------|------|---------------------|---------|----------|
| {如：Stripe} | {支付网关} | 是 | `STRIPE_SECRET_KEY` | 契约测试/L1: Mock, L2: sandbox, E2E: sandbox |
| {如：Twilio} | {短信服务} | 是 | `TWILIO_AUTH_TOKEN` | 契约测试/L1: Mock, L2: Mock, E2E: sandbox |
...
- **本地开发**: docker-compose 提供 PostgreSQL 等外部依赖
