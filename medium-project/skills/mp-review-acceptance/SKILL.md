...
1. 搜索现有 Issue：`gh issue list --label "type:acceptance" --search "验收预检 in:title" --state open --json number --jq '.[0].number'`
2. 如果未找到，创建：`gh issue create --title "验收预检" --label "type:acceptance" --body "跟踪验收预检过程。"`
3. 写入预检结果：`gh issue comment {ISSUE_NUMBER} --body "<预检结果>"`
4. 将同样的结果输出给技术负责人。
