...
12. 外部依赖清单完整（如有第三方 API：名称、用途、sandbox 模式、环境变量、各测试层级 Mock 策略）
13. 部署概要已说明（如影响模块设计）
...
1. 搜索现有 Issue：`gh issue list --label "type:architecture" --search "架构设计 in:title" --state open --json number --jq '.[0].number'`
2. 如果未找到，创建：`gh issue create --title "架构设计" --label "type:architecture" --body "跟踪系统架构的 Review 过程。"`
