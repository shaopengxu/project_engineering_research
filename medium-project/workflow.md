...
- **下限**：模块数 > 3，或模块间有多对多关系 / 复杂状态流转（审批流、状态机），或需要多人并行开发
- **可能过重**：虽然模块数 4-5，但模块间几乎独立（纯扇出结构），且只有 1 人开发 → 可简化流程
- **超出适用范围**：模块数 > 8、需要 > 5 个 agent 并行、技术负责人 > 70% 时间用于协调、模块间接口变更频繁（每周 > 3 次）
...
|------|------|
| 产品经理 | 定义需求，按模块组分批验收 |
| 技术负责人 | 技术选型，系统级 + 跨模块 review，并行调度协调（约 30-50% 时间），共享层变更审批 |
...
| Implementer agent | 按模块实现功能，只读 shared/，通过 Issue comment 沟通 |
...
- **存储与状态追踪**：GitHub Issues，多人可并行操作，无并发瓶颈
