...
小项目规模小，可以一口气做完，不需要中间迭代。中型项目模块多、复杂度高，开发过程中需求会变、优先级会调、甚至人员会换。没有迭代机制，项目后期发现方向偏了就来不及了。

**崩溃表现**：多个模块实现后才发现早期的架构决策有问题，但已经有 5 个模块基于它实现了；产品经理在所有开发完成后才第一次验收，发现核心流程不对。
...
7. **迭代/里程碑机制**：中型项目是否需要引入迭代节奏？如果引入，流程的 7 个步骤如何映射到迭代中？
