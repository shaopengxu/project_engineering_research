...
| 项目类型 | 业务模块举例 | 规模参考 |
|---------|------------|---------|
| CLI 工具 | 命令解析、业务逻辑、数据持久化 | 4-8 个命令 |
| REST API 服务 | 2-3 个资源的 CRUD + 资源间关联 | 10-15 个端点 |
| SDK / 工具库 | 核心功能、配置管理、输出格式化 | 10-20 个公开函数 |
| 消息消费者 | 事件处理、数据转换、存储写入 | 3-5 种事件类型 |
| 单页应用（无自建后端） | 2-3 个页面的状态管理和交互 | 3-5 个页面 |
| 浏览器扩展 | 内容脚本、后台逻辑、存储同步 | 1-2 个核心功能 |
| 自动化脚本 / 数据管道 | 数据采集、转换清洗、输出导出 | 2-3 个数据源 |
